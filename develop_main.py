    ...
    for event in longpoll.listen():
        ...
        if event.type == VkBotEventType.MESSAGE_NEW and \
            ...
            if event.type == VkBotEventType.MESSAGE_NEW and \
                ...
                usern, userf = vk.users.get(user_ids=(event.obj.message["from_id"]))[0]['first_name'], \
                               vk.users.get(user_ids=(event.obj.message["from_id"]))[0]['last_name']

                id_d[event.obj.message['from_id']] = {'name': ' '.join([usern, userf]),
                                                      'flag': False,
                                                      'flag_play': False,
                                                      'number_game': False,
                                                      'numb_gm_polz': False,
                ...
                text0 = f"Новый пользователь: {id_d[event.obj.message['from_id']]['name']}\n" \
                    f"Всего: {len(id_d)}"
                ...
                vk.messages.send(user_id=event.obj.message['from_id'],
                    ...
                    id_d[event.obj.message['from_id']]['words_game'][2] and not \
                    id_d[event.obj.message['from_id']]['help'][8] and id_d[event.obj.message['from_id']]['help'][19]:
                ...
                gl_add_word = [id, id_d[event.obj.message['from_id']]['add_word']]
                ...
                if event.type == VkBotEventType.MESSAGE_NEW and \
                    ...
                    text = f'''Вы хотите получить данные о городе {
                    id_d[event.obj.message['from_id']]['lg_lt_city'][2]}?\n Введите ДА или НЕТ'''
                    ...
                    vk.messages.send(user_id=event.obj.message['from_id'],
                        ...
                        id_d[event.obj.message['from_id']]['help'][
                            14]:  # подсказка-запрос о вводе буквы, на которую должны начинаться слова в режиме «на одну  букву»(игра «Слова»)
                ...
                ('добавить' in event.obj.message['text'].lower() or \
                 'не добавлять' in event.obj.message['text'].lower()) and wait_answ and \
                event.obj.message['peer_id'] == 2000000001:
            ...
            if 'добавить' in event.obj.message['text'].lower():
                ...
                text0 = Words.WordsGame().add_word(gl_add_word[1])

                text1 = f"Слово 💡{gl_add_word[1]}💡 было добавлено в словарь.\n"\

                if gl_add_word[1][0] not in id_d[gl_add_word[0]]['words'].keys():
                    id_d[gl_add_word[0]]['words'][gl_add_word[1][0]] = []

                vk.messages.send(peer_ids=['2000000001'],
                                 message='\n'.join([text0, text1]),
                                 random_id=random.randint(0, 2 ** 64))

            else:
                print('не добавляю')
                text = f"Слово 💡{gl_add_word[1]}💡 не было добавлено в словарь. "
            ...
            if not words_add_d[gl_add_word[0]]:   # проверка, предлагал ли пользователь еще какие-то слова
                ...
                id_add_sp.pop(id_add_sp.index(gl_add_word[0]))
            ...
            if event.type == VkBotEventType.MESSAGE_NEW and \
                ...
                event.obj.message['text'].lower() not in ['не добавлять', 'добавить']:

                text = f'''Пользователь {gl_add_word[0]} хочет добавить слово 💡{gl_add_word[1]}💡'''
                '''Добавить / Не добавлять'''
